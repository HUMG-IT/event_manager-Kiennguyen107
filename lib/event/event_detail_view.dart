...
class _EventDetailViewState extends State<EventDetailView> {
    ...
    if (pickedDate != null) {
      ...
      if (pickedTime != null) {
        setState(() {
          ...
          if (isStart) {
            //Thiet lap end time 1 tieng sau start time
  ...
  Future<void> _deleteEvent() async {
    ...
    if (!mounted) return;
    Navigator.of(context).pop(true); //Tra ve man hinh truoc do
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title:
            Text(widget.event.id == null ? 'Them su kien' : 'Chi tiet su kien'),
      ),
      body: Padding(
